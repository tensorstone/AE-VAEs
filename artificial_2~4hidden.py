...
newdata = pd.read_csv("tri-XYModel15_15simul.csv").values[17999:]
T = pd.read_csv("tri-Temp15_15.csv").values[17999:]
...
T = T[index]
x_train_1 =np.cos(newdata[0:2000]) 
x_test_1 = np.cos(newdata[2000:2100])
x_train_2 = np.sin(newdata[0:2000])
x_test_2 = np.sin(newdata[2000:2100])
...
lat_cos = np.zeros((1000,225))
lat_sin = np.zeros((1000,225))
for i in range(1000):
...
# display a 2D plot of the digit classes in the latent space  
x_test_encoded = encoder.predict(test_artificial[:100], batch_size=batch_size)  
plt.figure(figsize=(6, 6))  
#plt.scatter( x_test_encoded[:, 0] ,y_test , c=T_test)  
plt.scatter( x_test_encoded[:, 0] ,x_test_encoded[:, 1] , c=T_test)  
plt.hold()
x_test_encoded = encoder.predict(x_test[:100], batch_size=batch_size)  
#plt.scatter( x_test_encoded[:, 0] ,y_test , c=T_test)  
plt.scatter( x_test_encoded[:, 0] ,x_test_encoded[:, 1] , c=T_test)  
plt.colorbar()
plt.show()  
...
#encoded_imgs = encoder.predict(x_test)
decoded_imgs = vae.predict(test_artificial,batch_size=100)
size = 15
...
for i in range(1,n):
    # display original
    ax = plt.subplot(2,n,i)
    plt.imshow(test_artificial[i][:225].reshape(size, size))
    #plt.gray()
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
...
lat_cos_2 = np.zeros((1000,225))
lat_sin_2 = np.zeros((1000,225))
for i in range(1000):
...
# display a 2D plot of the digit classes in the latent space  
x_test_encoded = encoder.predict(test_artificial_2[:100], batch_size=batch_size)  
plt.figure(figsize=(6, 6))  
#plt.scatter( x_test_encoded[:, 0] ,y_test , c=T_test)  
plt.scatter( x_test_encoded[:, 0] ,x_test_encoded[:, 1] , c=T_test)  
plt.hold()
...
plt.figure(figsize=(6, 6))  
#plt.scatter( x_test_encoded[:, 0] ,y_test , c=T_test)  
plt.scatter( x_test_encoded[:, 1] ,x_test_encoded[:, 0] , c=T_test)  
plt.hold()
x_test_encoded = encoder.predict(test_artificial[:100], batch_size=batch_size)  
#plt.scatter( x_test_encoded[:, 0] ,y_test , c=T_test)  
